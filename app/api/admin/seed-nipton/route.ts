...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log("🛍️ Seeding Nipton Products...\n");
    ...
    console.log(`✅ DAW170 created with ${bag.variants.length} color variants\n`);
    ...
    console.log(`✅ CK7725 created with ${pants.variants.length} color variants\n`);
    ...
    console.log(`✅ CX8933 created with ${top.variants.length} color variants\n`);

    return NextResponse.json({
      success: true,
      message: "🎉 All Nipton products seeded successfully!",
      summary: {
        products: 3,
        variants: bag.variants.length + pants.variants.length + top.variants.length,
        details: [
          `✅ DAW170 Bag ($15) - ${bag.variants.length} colors`,
          `✅ CK7725 Pants ($15.64) - ${pants.variants.length} colors`,
          `✅ CX8933 Long Sleeve ($9.20) - ${top.variants.length} colors`,
        ],
      },
    });
  } catch (error) {
