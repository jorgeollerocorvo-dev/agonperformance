...
export default function CoachDirectory({
  ...
  lang: string;
  labels: { noMapLoc: string; from: string; perSession: string };
}) {
        ...
        {coaches.map((c) => {
          ...
          return (
            <li
              ...
              className={`rounded-3xl bg-white p-4 cursor-pointer transition border ${
                isActive
                  ? "border-[var(--primary)] ring-4 ring-[var(--primary-soft)]"
                  : "border-[var(--border)] hover:border-[var(--ink-subtle)]"
              }`}
            >
              <div className="flex items-start gap-3">
                <div className="w-14 h-14 rounded-2xl bg-[var(--primary-soft)] text-[var(--primary)] grid place-items-center text-xl font-bold shrink-0">
                  {c.photoInitial}
                </div>
